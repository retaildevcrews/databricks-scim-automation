        ...
        if (getCodeValueFromUrl()) {
            const authCodeElement = document.querySelector('#token input#auth-code');
            authCodeElement.value = getCodeValueFromUrl();
        }

        // Get key vault secrets, tenant ID, client ID, and client secret
        async function getKeyvaultSecrets(boxId) {
            try {
                addToLog('<br />Getting key vault secrets (tenant id, client id, and client secret)...');
                const keyvaultUrl = document.querySelector(`${boxId} input.keyvault-url`).value;
                if (!keyvaultUrl) {
        ...
        document.querySelector('#authcode button.keyvault-secrets').onclick = () => getKeyvaultSecrets('#authcode');
        document.querySelector('#token button.keyvault-secrets').onclick = () => getKeyvaultSecrets('#token');
        ...
        const loginElement = document.querySelector('#authcode button#login');
        ...
        let graphAccessTokenTimer;
        function setGraphAccessTokenTimer(initialExpiration) {
            if (graphAccessTokenTimer) {
                clearInterval(graphAccessTokenTimer);
            }
            ...
            graphAccessTokenTimer = setInterval(() => {
                const tokenExpirationElements =  document.querySelectorAll('span.token-expiration');
        ...
        // postGraphAccessToken needs code
        async function postGraphAccessToken() {
            try {
                addToLog('<br />Creating graph access token...');
                const code = getCodeValueFromUrl();
                ...
                const response = await fetch(`${document.location.origin}/graphAccessToken?code=${code}&tenantId=${tenantId}&clientId=${clientId}`, {
                    method: 'POST',
                ...
                const body = contentType.includes('json') ? await response.json() : await response.text();
                if (response.status === 200) {
                    // Display graph access token
                    const graphAccessTokenElements = document.querySelectorAll('input.graph-access-token');
                    graphAccessTokenElements.forEach(element => element.value = body.access_token);
                    addToLog(`Created graph access token: ${body.access_token}...`);
                    ...
                    setGraphAccessTokenTimer(body.expires_in);
                    setTimeout(() => {
                        clearInterval(graphAccessTokenTimer);
                        addToLog('<br />GRAPH ACCESS TOKEN HAS EXPIRED...');
                ...
                console.error(err);
                addToLog('Error while fetching graph access token...');
            }
        }
        document.querySelector('#token button#post-graph-access-token').onclick = postGraphAccessToken;
        
        async function postRefreshGraphAccessToken() {
            try {
                addToLog('<br />Refreshing graph access token...');
                const refreshToken = document.querySelector('#token input#refresh-token').value;
                const tenantId = document.querySelector('#token input.tenant-id').value;
                const clientId = document.querySelector('#token input.client-id').value;
                const clientSecret = document.querySelector('#token input.client-secret').value;
                if (!refreshToken || !tenantId || !clientId || !clientSecret) {
                    return logMissingInputs([{
                        value: refreshToken,
                        message: `Refresh token required to get graph access token...`,
                    }, {
                        value: tenantId,
                        message: `Key vault secrets required to get graph access token...`,
                    }, {
                ...
                const response = await fetch(`${document.location.origin}/refreshGraphAccessToken?tenantId=${tenantId}&clientId=${clientId}`, {
                    method: 'POST',
                    headers: {
        ...
        document.querySelector('#token button#post-refresh-token').onclick = postRefreshGraphAccessToken;
        ...
        async function postScimConnectorGalleryApp() {
            try {
                ...
                const response = await fetch(
                    ...
                    { method: 'POST', headers: { 'X-Graph-Access-Token': graphAccessToken } },
                );
                const contentType = response.headers.get('content-type');
                const body = contentType.includes('json') ? await response.json() : await response.text();
        ...
        async function getAadGroups() {
            try {
                ...
                const response = await fetch(`${document.location.origin}/aadGroups?filterAadGroupDisplayName=${encodeURIComponent(filterAadGroupDisplayName)}`, {
                    method: 'GET',
                    headers: { 'X-Graph-Access-Token': graphAccessToken }
                })
                const contentType = response.headers.get('content-type');
                const body = contentType.includes('json') ? await response.json() : await response.text();
                if (response.status === 200) {
        ...
        async function postValidateCredentials() {
            try {
                ...
                const response = await fetch(`${document.location.origin}/validateCredentials?${parameters}`, {
                    method: 'POST',
                    headers: { 'X-Graph-Access-Token': graphAccessToken, 'X-Databricks-Pat': databricksPat },
                });
